<template>
<s-dialog :show="show" :title='titleDialog'>
    <div class="home_wrapper">
        <q-form ref="form">
            ...
            />
            <div class="flex mt-base-15 items-center">
                <s-input
    ...
    //   MaxLength(9),
      ...
      MaxLength(2)
            ...
            <div class="flex mt-base-15">
                ...
                <div>
                    <q-checkbox v-model="noFlat"></q-checkbox>
                </div>
            </div>
            <s-input
                v-if="!noFlat"
...
export default defineComponent({
    ...
    setup(props, {
        ...
        const onSubmit = async () => {
            ...
            const houseFull = geoData.value.house + (geoData.value.houseLetter || '');

            let address = `г.${geoData.value.city} ул.${geoData.value.street} №${houseFull}`;

            if (!noFlat.value && geoData.value.flat) {
                address += ` кв.${geoData.value.flat}`;
            }

            emit('send-lock', address);
        }
